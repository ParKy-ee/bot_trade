...
def run_sim(df_data, guard_wick=False, wick_thresh=0.50, guard_overext=False, guard_adx=False, min_adx=18):
    trades = []
        ...
        for i in range(20, n - 10):
            ...
            if in_pos:
                ...
                if exit_price is not None:
                    ...
                    trades.append({'usd': net_usd, 'pips': net_pips, 'is_win': 1 if net_usd > 0 else 0})
                    in_pos = False
                continue
            ...
            if guard_wick:
                if action == 'BUY' and (row['prev_upper_wick_pct'] >= wick_thresh or row['upper_wick_pct'] >= wick_thresh):
                    continue
                if action == 'SELL' and (row['prev_lower_wick_pct'] >= wick_thresh or row['lower_wick_pct'] >= wick_thresh):
                    continue
            if guard_overext:
            ...
            tp_price = entry_price + tp_dist if action == 'BUY' else entry_price - tp_dist
...
base = run_sim(df, False, 0.50, False, False, 18)
base_trades = base['trades']

configs = [
    ("Baseline (No Guards)", False, 0.50, False, False, 18),
    ("Guard 1 Only: Wick Filter (>=50%)", True, 0.50, False, False, 18),
    ("Guard 1 Light: Wick Filter (>=60%)", True, 0.60, False, False, 18),
    ("Guard 2 Only: Overextended (RSI/ATR)", False, 0.50, True, False, 18),
    ("Guard 3 Only: ADX Chop (<18)", False, 0.50, False, True, 18),
    ("Guard 3 Light: ADX Chop (<14)", False, 0.50, False, True, 14),
    ("Guard 1(>=60%) + Guard 2", True, 0.60, True, False, 18),
